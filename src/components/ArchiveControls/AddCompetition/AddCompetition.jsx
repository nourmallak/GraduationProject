...
import styles from '../AddCompetition/AddCompetition.module.css';

export default function AddCompetition({ show, onHide, type, onSuccess }) {
  const [isLoading, setIsLoading] = useState(false);
  const validationSchema = Yup.object().shape({
    Name: Yup.string().required("Name is required"),
    Type: Yup.number().typeError("Type must be an integer").required("Type is required"),
  ...
  async function addCompetition(values, { setSubmitting, resetForm }) {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API}/Compitions/Add-Competition`,
        formData,
        {
          headers: {
